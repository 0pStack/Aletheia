...
* **Errors:** `400 BAD_REQUEST` (id is not a UUID), `404 NOT_FOUND` (unknown event, or another patient's), `409 PENDING` (the event is queued and not yet sealed into a block; retry after a few seconds), `429 RATE_LIMITED` (more than 60 requests a minute from one user).
* **Success Response (`data`):**
  ```json
  {
    "eventId": "b3e6c0f3-8f0a-4b68-9a3f-a3d8a1c92a91",
