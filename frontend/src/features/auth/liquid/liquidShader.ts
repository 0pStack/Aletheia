...
void main() {
  ...
  vec2 center = vec2(uLensCenter.x * aspect, uLensCenter.y) + uPointer * vec2(0.05, -0.05)
    + vec2(sin(uTime * 0.21), cos(uTime * 0.17)) * 0.012;
  float radius = 0.24 * minSide * (0.85 + 0.15 * uReveal);
  ...
  if (dist < 1.0) {
    ...
    float rim = 1.0 - z;
    vec2 bend = normal.xy * rim * radius * 3.2;
    vec2 magnified = center - (st - center) * 0.55;
    ...
    float specular = pow(max(dot(reflect(-light, normal), vec3(0.0, 0.0, 1.0)), 0.0), 90.0);
  ...
  color += (grain(gl_FragCoord.xy) - 0.5) * 0.07;
